...
const VendorDashboard = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gray-100 min-h-screen">
      ...
      {/* Listings Section */}
      <div className="mt-6 bg-white p-6 shadow-md rounded-lg">
        <h2 className="text-2xl font-semibold text-gray-700">Your Recent Listings ({listings.length})</h2>
        {listings.length === 0 ? (
        ...
        {bookings.length === 0 ? (
          ...
          <div className="mt-4 space-y-4">
            {bookings.slice(0, 5).map(booking => (
              <div key={booking._id} className="p-4 bg-gray-50 border rounded-lg shadow-sm">
                <p className="text-gray-800 font-semibold">Booking ID: {booking._id}</p>
                <p className="text-gray-600">Status: {booking.status}</p>
                <p className="text-gray-600">Date: {new Date(booking.bookingDate).toLocaleDateString()}</p>
                <Link to={`/vendor/bookings/${booking._id}`} className="text-blue-600 mt-2 inline-block hover:underline">View Details</Link>
              </div>
            ))}
          </div>
        )}
